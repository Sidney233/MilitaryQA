...
class MilitaryGraph:
    def __init__(self):
        ...
        self.datapath = os.path.join(cur, 'json_data/military.json')
    ...
    def load_weapons(self):
        weapons = []
        for record in open(self.datapath, encoding="utf-8"):
    ...
    def question_parser(self, question):
        ...
        parser_dict['pattern'] = [flag for wd, flag in wds if
                                  flag in ['n_attr', 'n_time', 'n_big', 'n_small', 'n_unit', 'n_country', 'n_compare',
                                           'n_most', 'n_weapon']]
        # parser_dict['wds'] = wds
        return parser_dict
    ...
    def search_answer(self, parser_dict):
        ...
        # 实体、实体属性相间隔
            ...
            targets1 = [self.attribute_dict.get(weapon) for indx, weapon in enumerate(parser_dict.get('n_attrs')) if
                        indx < n_indxes[1] - 1]
            targets2 = [self.attribute_dict.get(weapon) for indx, weapon in enumerate(parser_dict.get('n_attrs')) if
                        indx >= n_indxes[1] - 1]
    ...
    def query_mongo_attr(self, search_data):
        ...
        for search in search_data:
            ...
            for res in self.col.find(condition):
                result.append([res.get('名称') + target + ':' + str(res.get(target, 'null')) for target in targets if
                               res.get(target, 'null') != 'null'])
        return result
    ...
    def qa_main(self, question):
        ...
        results, answer_type = self.search_answer(parser_dict)
        res = ""
        res_type = "phase_error"
        res_num = 0
        if len(results) == 0:
            res = "对不起，目前暂时还无法回答此类问题..."
            res_type = "phase_error"
            res_num = 0
        else:
            res_type = "success"
            ...
            if answer_type == "introduction":
                ...
                for result in results:
                    ...
                    res += "，"
                res = res.strip("，")
            elif answer_type == "multi_answer" or answer_type == "multi_entry" or answer_type == "compare":
                for result in results:
                    for i in result:
                        parts = i.split(":")
                        res += parts[0] + "是" + parts[1] + "，"
                res = res.strip("，")
                res += "。"
        return res, res_type, res_num
...
if __name__ == '__main__':
    ...
    while 1:
        ...
        res, res_type, res_num = handler.qa_main(question)
        print(res)
        print(res_type)
        print(res_num)
